...
import com.swbg.mlivestreaming.bean.GiftBean
    ...
    override fun onBindViewHolder(holder: CacheViewHolder, position: Int) {
        holder.itemView.apply {
            ...
            if (get(position).isSelect) {
                context?.let { it1 -> Glide.with(it1).load(get(position).animation).placeholder(R.mipmap.default_avatar).into(iv_avatar) }
            } else {
            context?.let { it1 -> Glide.with(it1).load(get(position).image).placeholder(R.mipmap.default_avatar).into(iv_avatar) }
            }
