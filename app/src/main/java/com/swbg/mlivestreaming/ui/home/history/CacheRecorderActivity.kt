package com.swbg.mlivestreaming.ui.home.history
...
class CacheRecorderActivity : MBaseActivity() {
    ...
    /*    rv_history.apply {
            layoutManager = LinearLayoutManager(this@CacheRecorderActivity)
            adapter = mAdapter
        }*/
        initData()
        setListener()
        ...
        CachedStatusWrapper().apply {
            client = object : CachedAutoRefreshAdapter<VideoBean>() {
                override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): CacheViewHolder {
                    return CacheViewHolder(LayoutInflater.from(this@CacheRecorderActivity)
...
//                        Glide.with(context).load(data.faceImageUrl).into(iv_video_image)
                        ...
                        iv_video_image.singleClick {
                            startActivityWithTransition(VideoPlayActivity.open(this@CacheRecorderActivity,data.id))
                        }
                        tv_title.singleClick {
                            startActivityWithTransition(VideoPlayActivity.open(this@CacheRecorderActivity,
                                data.id))
                        }
    ...
    companion object {
        fun open(context: Context): Intent {
            return Intent(context, CacheRecorderActivity::class.java)
        }
    }
}
